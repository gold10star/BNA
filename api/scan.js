export default async function handler(req, res) {
  ...
  const prompt = `You are validating and extracting data from an ATM/BNA Host Total receipt.

The receipt may be rotated — read in any orientation.

== STEP 1: VALIDATE FIRST ==
Before extracting anything, check if this image is a genuine ATM/BNA Host Total receipt.

A valid receipt MUST have ALL of the following:
- An ATM ID field starting with ATM or BNA ID
- A REF NO field
...
If the image is NOT a valid Host Total receipt (e.g. it is a random photo, a different bank's receipt, a transaction slip, a selfie, a document, blank, blurry/unreadable, or missing the required fields), return ONLY this JSON:
...
- "No ATM ID was detected in the image."
  ...
  try {
    ...
    try {
      ...
      if (parsed.valid === false) {
        return res.status(422).json({ error: 'invalid_receipt', reason: parsed.reason || 'This does not appear to be a valid Host Total receipt.' });
      }
